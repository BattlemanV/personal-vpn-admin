...
_XRAY_TRAFFIC_CACHE: Dict[str, Dict[str, int]] = {}
_XRAY_TRAFFIC_CACHE_TS: float = 0
...
async def lifespan(app):
    ...
    if WG_VARIANT == "xray":
        try:
            ...
            print(f"[lifespan] sync_xray_config: {e}")
...
def get_xray_traffic() -> Dict[str, Dict[str, int]]:
    global _XRAY_TRAFFIC_CACHE, _XRAY_TRAFFIC_CACHE_TS, _XRAY_TRAFFIC_ACCUM
    now = time.time()
    ...
    with _XRAY_TRAFFIC_LOCK:
        for email, data in result.items():
            prev = _XRAY_TRAFFIC_ACCUM.get(email, {"downlink": 0, "uplink": 0})
            _XRAY_TRAFFIC_ACCUM[email] = {
                "downlink": max(prev["downlink"], data["downlink"]),
                "uplink": max(prev["uplink"], data["uplink"]),
            }
        _XRAY_TRAFFIC_CACHE = dict(_XRAY_TRAFFIC_ACCUM)
        _XRAY_TRAFFIC_CACHE_TS = now
    return _XRAY_TRAFFIC_CACHE
    ...
    if isinstance(clients, dict):
        for client_id, client in clients.items():
            ...
            if live:
                ...
                # Xray clients without WG dump: use Xray API stats for online/traffic
                xray_uid = ""
                if WG_VARIANT == "xray":
                    xray_uid = str(client.get("xray_uuid", "") or "")
                    if xray_uid:
                        traffic_all = get_xray_traffic()
                        pt = traffic_all.get(xray_uid, {})
                        dl = pt.get("downlink", 0)
                        ul = pt.get("uplink", 0)
                        if ul or dl:
                            transfer_rx = ul
                            transfer_tx = dl
                            transfer_total = ul + dl
                            transfer_rx_human = bytes_to_human(ul)
                            transfer_tx_human = bytes_to_human(dl)
                            transfer_total_human = bytes_to_human(ul + dl)
                            now_ts = int(time.time())
                            save_peer_last_seen(xray_uid, now_ts)
                            latest_handshake = now_ts
                            online = True

                traffic_key = xray_uid if xray_uid else public_key
                saved_last_seen = get_peer_last_seen(traffic_key)
                if not online:
                    if saved_last_seen:
                        latest_handshake = saved_last_seen
                        online = (int(time.time()) - saved_last_seen) < ONLINE_THRESHOLD_SECONDS
                latest_handshake_text = handshake_to_text(latest_handshake) if latest_handshake else "never"
                if traffic_key:
                    period = get_period_traffic(traffic_key, transfer_rx, transfer_tx, online)
                else:
                    period = {}
            ...
            peers.append({
                "id": public_key[:12] if public_key else client_id[:12],
                "name": peer_name,
                "ip": address,
...
def get_client_xray_uuid(client_id: str) -> str:
    try:
        item = get_client(client_id)
        client = item["client"]
        ...
        for inbound in xray_cfg.get("inbounds", []):
            ...
            if inbound.get("streamSettings", {}).get("security") == "reality":
                inbound["settings"]["clients"] = [
                    {"id": uid, "email": uid, "flow": "xtls-rprx-vision"} for uid in enabled_uuids
                ]
            else:
                inbound["settings"]["clients"] = [
                    {"id": uid, "email": uid} for uid in enabled_uuids
                ]

        tmp = XRAY_JSON + ".tmp"
        with open(tmp, "w") as f:
            json.dump(xray_cfg, f)
        os.rename(tmp, XRAY_JSON)

        pid = subprocess.run(["pidof", "xray"], capture_output=True, text=True).stdout.strip()
        if pid:
...
def diagnostics(x_api_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    ...
    if WG_VARIANT == "awg":
        ...
        if awg_transfer:
            ...
            if rx_bytes or tx_bytes:
                awg_info["rx_bytes"] = rx_bytes
                awg_info["tx_bytes"] = tx_bytes
    ...
    return {
        ...
        "disk_used": disk_used,
        "awg": awg_info if awg_info else None,
    }
...
def peer_traffic_days(client_id: str, x_api_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    ...
    traffic_key = peer.get("public_key") or peer.get("pubkey") or ""
    if not traffic_key and WG_VARIANT == "xray":
        try:
            item = get_client(client_id)
            traffic_key = item["client"].get("xray_uuid", "") or ""
        except Exception:
            pass
    if not traffic_key:
        return {"days": []}
...
def peer_traffic_hours(client_id: str, date: Optional[str] = Query(default=None), x_api_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    ...
    with sqlite3.connect(TRAFFIC_DB_FILE, timeout=10) as db:
        rows = db.execute("""
            ...
            ORDER BY period_key ASC
        """, (traffic_key, date + "%")).fetchall()
        if rows:
            return {"hours": [{"h": r[0].split("-")[-1], "rx": int(r[1]), "tx": int(r[2])} for r in rows]}
        if date == today_key():
            day = read_traffic_total(db, traffic_key, "day", date)
